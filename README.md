...
- **发送可靠性（重点）**：
  ...
  - **仍可能失败并返回 `enqueued:true`** 的场景：平台发送风控（短时高频发送触发 `prepare failed`）、网络异常、fallback 也失败等——此时消息**进入待补发队列**
  - **风控自动冷却**：连续 3 次发送失败（`prepare failed`）触发**全局暂停发送 3 分钟**（期间不再向 iLink 打请求，避免加重风控），到期自动恢复；暂停期间的消息进待补发队列
