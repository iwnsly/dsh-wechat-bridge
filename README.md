...
- **发送可靠性（重点）**：
  - 优先携带 `context_token`（保持会话上下文）；若其过期（`ret=-2`），桥接**自动 fallback 为不带 token 发送**（实测可正常送达），主动/定时推送基本不受凭证时效限制
  - 每次发送带 **15 秒超时 + 2 次重试**（网络/平台慢时会自动重试）
  - **仍可能失败并返回 `enqueued:true`** 的场景：微信会话失活（长时间无微信消息、只操作网页端，iLink 返回 `prepare failed`）、网络异常、fallback 也失败等——此时消息**进入待补发队列**
  - **会话失活处理**：`prepare failed` 通常是 iLink **会话失活**（非频率风控）——**发一条微信消息即可立即恢复**（桥接收到消息自动解除冷却并补发）；连续失败会临时冷却 3 分钟避免空打请求
  - **补发时机**：① 你下次在微信发一条消息后，桥接用新凭证**自动补发最后一条**未送达的回复（带「（补发）」标注）；② **后台每 10 分钟自动尝试补发一次**（会话一恢复即补上，定时发送不依赖你发消息）；中间被节流/失败的中间进度不重复推
...
- 限制：单文件 ≤ 20MB；凭证过期时自动 fallback 为无 token 发送（同 `/api/send`）
- **注意**：文件发送失败**不进待补发队列**（与文本 `/api/send` 不同）——调用方需检查返回并自行重试；若因凭证/风控失败，先在微信发一条消息刷新凭证后重试。
