...
| **空白/空格消息** | **特殊指令「建立连接」**：补发当前会话积压的最新消息；会话空闲则回发其最近一条记录，进行中则回当前任务状态（不进入 DSH 对话） |
...
- **阶段回复（流式）**：DSH 每完成一个生成步骤（`assistant/message` 定稿）就立即把该步文本发到微信，不用等全部生成完；最终文本若已随阶段发出则不重复发送。发送失败不记"已送达"，**中间阶段失败也会进入待补发队列**（补发时按会话取最新一条），最终回复自动补发——**内容不丢**。
...
- **发送可靠性（重点）**：
  ...
  - **补发时机**：切换会话成功后、你发微信消息后、以及后台每 10 分钟，都会尝试补发——每次都只补发**当前会话**积压的**最后一条**（带「（补发）」标注），不会回放旧会话或多条中间进度；中间/最终失败的文本都会进入队列（同会话只保留最新），文件发送失败不进入此队列。
