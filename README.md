...
- **凭证与时效**：发送时优先携带 `context_token`（保持会话上下文）；若其过期（`ret=-2`），桥接**自动 fallback 为不带 token 发送**（实测可正常送达），因此**主动/定时推送不受凭证时效限制**。仅在极端情况下（fallback 也失败）返回 `enqueued:true`，等你下次发微信消息后自动补发。
